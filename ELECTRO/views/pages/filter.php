...
if(isset($_GET['filter_arenda'])) {
...
} elseif(isset($_GET['filter_ports'])) {
    $year = $_GET['year'];
    $id = $_GET['id_object'];
    ...
    $query = mysqli_query($db, "SELECT  jan.mount AS janm, jan.used as janu, f.mount AS febm, f.used as febu, mar.mount AS marm, mar.used as maru, apr.mount AS aprm, apr.used as apru, may.mount AS maym, may.used as mayu, jun.mount AS junm, jun.used as junu, jul.mount AS julm, jul.used as julu, aug.mount AS augm, aug.used as augu, s.mount AS sepm, s.used as sepu, o.mount AS octm, o.used as octu, n.mount AS novm, n.used as novu, d.mount AS decemberm, d.used as decemberu  FROM `object` obc LEFT JOIN `$jan` jan ON (jan.id_object = obc.id_object) LEFT JOIN `$feb` f ON (f.id_object = obc.id_object) LEFT JOIN `$mar` mar ON (mar.id_object = obc.id_object) LEFT JOIN `$apr` apr ON (apr.id_object = obc.id_object) LEFT JOIN `$may` may ON (may.id_object = obc.id_object) LEFT JOIN `$jun` jun ON (jun.id_object = obc.id_object) LEFT JOIN `$jul` jul ON (jul.id_object = obc.id_object) LEFT JOIN `$aug` aug ON (aug.id_object = obc.id_object) LEFT JOIN `$sep` s ON (s.id_object = obc.id_object) LEFT JOIN `$oct` o ON (o.id_object = obc.id_object) LEFT JOIN `$nov` n ON (n.id_object = obc.id_object) LEFT JOIN `$dec` d ON (d.id_object = obc.id_object) WHERE obc.id_object = '$id'");
...
<?php
    if(isset($_GET['filter_arenda'])) {
        ...
        require_once('views/components/filters/filterMount.php');
        include('views/modal/modalForPorts.php');
    }
